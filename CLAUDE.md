...
| A check that re-runs on a ready pull request is not CI that reported nothing | the CI wait is timed from when this head was first seen pending — `delivery.ci_pending_head`/`ci_pending_since` (v11), on the injected clock — and cleared whenever CI is not pending, never from crewd's own push (#105) | `a_check_re_run_on_a_ready_pull_request_does_not_hand_it_off` |
