...
| A continuation keeps its place in the milestone order | a retry queued by a `Continue` verdict — the agent's or the gate's — records the state whose slot it holds (`retry.reserved_state`, v10), and `worker_slots`/`state_slots` count it for the length of its delay, against the worker holding its session; a failure backoff, a delivery hand-back and a rate-limit release reserve nothing; the reservation lives on the retry row, so every path deleting the row ends it, and `dispatch_due_retries` re-reads a waiting reservation each tick so a ticket closed or unlabelled during the delay frees its slot at once — while an id the tracker merely omits keeps its reservation until due, one omission being the blip `refresh_miss_grace` exists for; a due reservation counts only the reservations ahead of it in due order, so ones oversubscribing a limit after a state change dispatch earliest-first instead of deferring each other forever; the snapshot publishes `reserved` and `Row::holds_slot` (#86) | `a_continuing_issue_keeps_its_slot_through_its_continuation_delay`, `a_continuation_holds_its_slot_in_its_own_state_limit`, `a_failure_backoff_does_not_hold_a_slot`, `a_continuation_whose_ticket_went_terminal_releases_its_reserved_slot`, `a_waiting_continuation_survives_one_tracker_omission_with_its_slot`, `reservations_oversubscribing_a_state_limit_dispatch_in_due_order_rather_than_deadlocking` |
| An account-wide rate limit is not any one issue's failure, nor any other worker's | a rejected `rate_limit_event` releases the claim (`Store::release_for_rate_limit`, not `release`) without charging an attempt or the identical-failure streak, and pauses dispatch to the run's own worker until `resets_at` rather than scheduling a per-issue retry; the pause is keyed by worker name, so the other workers keep dispatching and `Snapshot::rate_limit_pauses` names each paused one (#119) | `a_rate_limit_pauses_dispatch_rather_than_quarantining_the_issues_it_interrupted`, `a_rate_limit_on_one_worker_does_not_pause_dispatch_to_the_other` |
| A continuation stays on the worker that holds its session | the run row records its worker (v13) and `Store::session_worker` names the one that ran the current session; `pick_worker` gives a pinned issue its own worker or nothing, so a continuation whose worker is full or paused waits rather than jumping provider, and `launch` never resumes a session another worker holds (#119) | `a_continuation_stays_on_the_worker_that_holds_its_session` |
| Dispatch overflows in worker order | `pick_worker` gives an unpinned issue the first worker in `[[workers]]` order that is not paused and has a free slot; a config without `[[workers]]` is the one-element list of `[worker]` at `agent.max_concurrent` | `an_issue_goes_to_the_first_worker_with_a_free_slot`, `a_single_worker_config_behaves_exactly_as_before` |
...
| A gate cannot outrun the concurrency limit | `worker_slots` and `state_slots` count `gating` as well as `running`, because a gate is a build on this host and the claim is held across it; a gating run and a reserved continuation count against the worker that produced them (#119) | `a_gating_run_still_holds_its_concurrency_slot_so_gates_cannot_accumulate`, `a_gating_run_holds_the_slot_of_the_worker_that_produced_it` |
...
| A finding left only in a review's summary is worked like an inline comment | `step_delivery` adds `summary_findings` — each review on the current head from a `summary_reviewers` login, or `CHANGES_REQUESTED` from anyone, whose body says more than "Findings: None" — to the open comments, keyed `review-<id>`, so the same verdict table settles it once, the same round bounds charge it, and an unsettled one holds or leaves `ready`; its verdict is a pull request comment quoting it, and it has no thread to resolve (#126) | `a_finding_only_in_a_review_summary_is_handed_back_to_the_agent`, `a_review_after_ready_with_a_summary_finding_reopens_the_round`, `a_settled_summary_finding_is_not_handed_back_again`, `a_summary_that_says_findings_none_and_nothing_else_hands_nothing_back`, `a_changes_requested_review_with_only_a_summary_is_handed_back` |
